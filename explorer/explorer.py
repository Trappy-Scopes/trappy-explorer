...
class Explorer:
    ...
    def __init__(self, datadirs, safemode=True):
        ...
        expfiles = [os.path.join(file, "experiment.yaml") for file in datadirs]
        expfiles = [file for file in expfiles if os.path.isfile(file)]
        notexpfiles = [file for file in expfiles if not os.path.isfile(file)]
        ...
        for i, expfilename in enumerate(self.expfiles):
            ...
            try:
                ...
                print(f"[red]Error: experiment.yaml failed to parse. Please consider using safemode=True, \
                      or maybe the file is damaged. Please try to open the file manually to check. :[default] {eid}")
                continue
            
            ## Update indexes
            self.eidlist.append(eid)
            self.expmap[expfilename] = eid
            self.datadir_map[eid] = self.datadirs[i]  ## Save a map of eid: absolute_path_to_dataset_directory
    ...
    def get_converted_dir(self, eid=0):
        ...
        Return the corresponding analysis directory.
        """
        eid = self.__eid_by_index__(eid=0)
